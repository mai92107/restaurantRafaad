...
export const getMenuItemByRestaurantId = ( reqData ) => {
    return async (dispatch) => {
        ...
        try {
            const { data } = await api.get(`/api/food/restaurant/${reqData.restaurantId}?vegetarian=${reqData.vegetarian}&notVeg=${reqData.notVeg}&seasonal=${reqData.seasonal}&food_category=${reqData.foodCategory}`, {
                headers: {
                    Authorization: `Bearer ${reqData.jwt}`
                }
            });
            console.log('menu', data,reqData.foodCategory);
